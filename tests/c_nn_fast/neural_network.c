...
t_layer		*new_layer(int nb_neurons, int nb_weights_per_neuron)
{
	...
	if (!(res->neurons = (t_neuron**)malloc(sizeof(t_neuron*) * nb_neurons)))
		malloc_error();
	if (!(res->neurons_values = (double*)malloc(sizeof(double) * (nb_neurons + 1))))
		malloc_error();
	nb_neurons--;
	res->neurons_values[nb_neurons + 1] = -1.0;
	while (nb_neurons >= 0)
	{
		res->neurons[nb_neurons] = new_neuron(nb_weights_per_neuron);
		res->neurons_values[nb_neurons] = 0.0;
		nb_neurons--;
	}
	return (res);
}
...
void	calculate_layer(t_neural_net *neural_net, int layer_nb, double (*activation_func)(double))
{
	...
	{
		calculate_neuron_value(neural_net->layers[layer_nb], i, neural_net->layers[layer_nb - 1], activation_func);
		i++;
	}
}

void	update_layer(t_neural_net *neural_net, int layer_nb, double (*activation_func)(double), int weight_nb, int prev_val)
{
	int		i;
	...
	{
		//calculate_neuron_value(neural_net->layers[layer_nb], i, neural_net->layers[layer_nb - 1], activation_func);
		neural_net->layers[layer_nb]->neurons_values[i] -= prev_val * neural_net->layers[layer_nb]->neurons[i]->weights[weight_nb];
		neural_net->layers[layer_nb]->neurons_values[i] += neural_net->layers[layer_nb - 1]->neurons_values[weight_nb] * 
														neural_net->layers[layer_nb]->neurons[i]->weights[weight_nb];
		i++;
	}
}
...
double	*get_output(t_neural_net *neural_net, double (*activation_func)(double), double *ret)
{
	...
	{
		ret[i] = neural_net->layers[neural_net->nb_layers - 1]->neurons_values[i];
		i++;
	}
	return (ret);
}
...
double	*update_output(t_neural_net *neural_net, double (*activation_func)(double), double *ret,
														int layer_nb, int neuron_nb, int weight_nb, int prev_val)
{
	int		i;
	int		old_val;

	(void)old_val;
	//calculate_neuron_value(neural_net->layers[layer_nb], neuron_nb, neural_net->layers[layer_nb - 1], activation_func);
	old_val = neural_net->layers[layer_nb]->neurons_values[neuron_nb];
	neural_net->layers[layer_nb]->neurons_values[neuron_nb] -= neural_net->layers[layer_nb - 1]->neurons_values[weight_nb] * prev_val;
	neural_net->layers[layer_nb]->neurons_values[neuron_nb] += neural_net->layers[layer_nb - 1]->neurons_values[weight_nb] * 
														neural_net->layers[layer_nb]->neurons[neuron_nb]->weights[weight_nb];
	i = layer_nb + 1;
	while (i < neural_net->nb_layers)
	{
		calculate_layer(neural_net, i, activation_func);
		//update_layer(neural_net, i, activation_func, neuron_nb, old_val);
		i++;
	}
	i = 0;
	while (i < neural_net->layers[neural_net->nb_layers - 1]->nb_neurons)
...
void	set_first_layer(t_neural_net *neural_net, double *input)
{
	...
	{
		neural_net->layers[0]->neurons_values[i] = input[i];
		i++;
	}
}
...
void	calculate_neuron_value(t_layer *layer, int neuron_nb, t_layer *previous_layer, double (*activation_func)(double))
{
	...
	if (layer->neurons[neuron_nb]->nb_weights == 0)
		return ;
	layer->neurons_values[neuron_nb] = -1.0 * layer->neurons[neuron_nb]->weights[0];
	i = 1;
	while (i + 10 <= layer->neurons[neuron_nb]->nb_weights)
	{
		layer->neurons_values[neuron_nb] += previous_layer->neurons_values[i - 1] * layer->neurons[neuron_nb]->weights[i] + 
							previous_layer->neurons_values[i] * layer->neurons[neuron_nb]->weights[i + 1] +
							previous_layer->neurons_values[i + 1] * layer->neurons[neuron_nb]->weights[i + 2] +
							previous_layer->neurons_values[i + 2] * layer->neurons[neuron_nb]->weights[i + 3] +
							previous_layer->neurons_values[i + 3] * layer->neurons[neuron_nb]->weights[i + 4] +
							previous_layer->neurons_values[i + 4] * layer->neurons[neuron_nb]->weights[i + 5] +
							previous_layer->neurons_values[i + 5] * layer->neurons[neuron_nb]->weights[i + 6] +
							previous_layer->neurons_values[i + 6] * layer->neurons[neuron_nb]->weights[i + 7] +
							previous_layer->neurons_values[i + 7] * layer->neurons[neuron_nb]->weights[i + 8] +
							previous_layer->neurons_values[i + 8] * layer->neurons[neuron_nb]->weights[i + 9];
		i += 10;
	}
	while (i < layer->neurons[neuron_nb]->nb_weights)
	{
		layer->neurons_values[neuron_nb] += previous_layer->neurons_values[i - 1] * layer->neurons[neuron_nb]->weights[i];
		i++;;
	}
	layer->neurons_values[neuron_nb] = activation_func(layer->neurons_values[neuron_nb]);
	//printf("val : %f\n", layer->neurons_values[neuron_nb]);
}
...
/*void	calculate_neuron_value(t_layer *layer, int neuron_nb, t_layer *previous_layer, double (*activation_func)(double))
{
	if (layer->neurons[neuron_nb]->nb_weights == 0)
		return ;
	layer->neurons_values[neuron_nb] = activation_func(cblas_ddot(previous_layer->nb_neurons + 1, layer->neurons[neuron_nb]->weights, 1, previous_layer->neurons_values, 1));
	//printf("val : %f\n", layer->neurons_values[neuron_nb]);
}*/
															...
															double ori_cost, double *ret)
{
	...
	update_output(neural_net, activation_func, ret, layer_nb, neuron_nb, weight_nb, ori_val);
	cost = cost_func(ret, expected_output, neural_net->layers[neural_net->nb_layers - 1]->nb_neurons);
	tmp = neural_net->layers[layer_nb]->neurons[neuron_nb]->weights[weight_nb];
	neural_net->layers[layer_nb]->neurons[neuron_nb]->weights[weight_nb] = ori_val;
	update_output(neural_net, activation_func, ret, layer_nb, neuron_nb, weight_nb, tmp);
