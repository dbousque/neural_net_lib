...
void	print_gradient(double ***gradient, t_neural_net *neural_net)
{
	int		x;
	int		y;
	int		z;
...
void	gradient_descent(t_neural_net *neural_net, double **inputs, double **exp_outs, int nb_inputs, int mini_batch_size, int nb_epochs, double learning_rate)
{
	...
	while (i < nb_epochs)
	{
		...
		while (x < nb_inputs)
		{
			...
			//print_gradient(gradient, neural_net);
...
int		main(void)
{
	...
	double			**exp_outs;
	int				nb_inp;
	int				nb_neurons_per_layer[] = {16 * 18, 30, 26};
	t_neural_net	*neural_net;
	//int				lol;

	srand(time(NULL));
	get_all_images_in_dir("../letters", &nb_inp, 18, 16, &images, &exp_outs);
	if (DISTORTIONS)
	{
		generate_distortions(&images, &exp_outs, nb_inp, 16, 18);
		//lol = write_image_to_file(images[4], 16, 18, "ori_m.ppm");
		//lol = write_image_to_file(images[4 + 26], 16, 18, "m_left.ppm");
		//lol = write_image_to_file(images[4 + 26 * 2], 16, 18, "m_right.ppm");
		//lol = write_image_to_file(images[4 + 26 * 3], 16, 18, "m_down.ppm");
		//lol = write_image_to_file(images[4 + 26 * 4], 16, 18, "m_up.ppm");
		//(void)lol;
	}
	neural_net = new_neural_net(nb_neurons_per_layer, 3, ACTIVATION_FUNC, COST_FUNC);
	printf("ori_score : %.2f%%\n", score(neural_net, images, exp_outs, nb_inp));
	gradient_descent(neural_net, images, exp_outs, nb_inp, MINI_BATCH_SIZE, NB_EPOCHS, LEARNING_RATE);
	return (0);
}*/
